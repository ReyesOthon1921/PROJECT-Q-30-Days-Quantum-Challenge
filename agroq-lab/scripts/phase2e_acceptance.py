...
def evaluate(db_path: Path, deployment_ready: bool) -> dict[str, Any]:
    ...
    if not db_path.exists():
        ...
        add("P2E-OUTAGE", "24-hour outage acceptance", outage > 0, f"passed records: {outage}")
        add("P2E-BACKUP", "Verified backup recovery", backups > 0, f"verified backups: {backups}")
        ...
        add("P2E-AUDIT", "Audit trail evidence", audit > 0, f"audit events: {audit}")
...
def main() -> int:
    ...
    report = evaluate(Path(args.db), bool(gateway_configuration()["deployment_ready"]))
